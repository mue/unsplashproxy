...
fastify.get('/getImage', async (_req, res) => {
  const data = await (await fetch(`https://api.unsplash.com/photos/random?client_id=${process.env.UNSPLASH_KEY}&query=nature&content_filter=high&featured=true&orientation=landscape`).send()).json();
  res.send({
  ...
  await fetch(`${data.links.download_location}?client_id=${process.env.UNSPLASH_KEY}`).send();
});
